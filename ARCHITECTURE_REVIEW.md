...
| ⚠️ W17 | `wc_context` + `odds.py` | **ACCEPTED BY DESIGN (documented)** — XGB sees the 1X2 market; the λ blend uses AH/O-U (different markets, correlated information). With C5 fixed, XGB now actually receives WC-26 1X2 odds. Revisit `_MARKET_BLEND=0.20` if backtests suggest over-weighting | — |
