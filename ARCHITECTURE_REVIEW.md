...
| ✅ W18 | `generate_submission.py` | **FIXED** — multi-restart coordinate descent (greedy + 7 random) against local optima, **plus the R8 second pass** (third-place cross-group coupling, 2026-06-15) that credits tipped best-8 thirds: +21.9 optimiser EV. See roadmap R8. | — |
...
**✅ R6 (DONE 2026-06-15). Canonical-name registry + ingress validation.** `data/wc2026.normalise()` is the single canonical name function (confirmed sufficient). Added `data/validation.py::validate_wc2026_coverage()` — a startup check asserting each of the 48 WC teams has ≥30 training matches **and** a hit in every prediction-time lookup (rankings, odds, transfermarkt, fc26, api_form, injuries, xG), resolving each source's own spelling back to canonical. Prints a coverage matrix and warns loudly on any gap; critical gaps (matches/rankings/odds) can hard-fail with `strict=True`. Wired non-fatally into `pipeline.step_load` and `predict_wc2026.train_model`. Result on current data: **48/48 covered on all critical sources; only New Zealand misses xG** (the one known acceptable gap). This is exactly the guard that would have caught the three silent name-miss bugs (C4/C5/B11). Tests in `tests/test_validation.py`.
...
**✅ R8 (DONE 2026-06-15). Joint submission optimisation — third-place coupling.** The third-place pool couples groups: which 8 thirds advance depends on tips across all 12 groups. The 1st-pass per-group optimiser credited **only top-2** advancement, ignoring third-place bonus entirely. R8 adds a **second pass** (`generate_submission.optimise_group_scores(third_ctx=…)`, default on; `--no-two-pass` to disable): after pass 1, each group's tipped 3rd-place team is credited 3·P(advance) when, given the other 11 groups' fixed tipped thirds, it ranks in the **global best-8** — so group A's scorelines now respond to B–L. Measured on the live submission: total optimiser EV **227.2 → 249.1 (Δ +21.9)** — ~22 expected league points of third-place advancement the old version left on the table (the single biggest competition-points lever; log-loss is unaffected — this optimises submission points, not calibration). Note: the conceptually "purest" form (self-conditioning `estimate_advance_probs` on chosen scorelines) is EV-incorrect — reality isn't changed by your tips — so R8 instead credits tips by the model's unconditioned P(advance), which is the correct expected-points objective. Supersedes W18 (multi-restart was the partial fix).

**❌ R9 (TESTED & REJECTED 2026-06-15). Bayesian-shrunk form/SoS.** Empirical-Bayes shrinkage of the form/SoS window means toward the population mean, weighted by match count (`features/form.py` + `sos.py`, flag `USE_FORM_SHRINKAGE`, K=5). WC ablation (`scripts/roadmap_ablation.py`, fw=0.8): **worsened all 3 folds** (avg 1.0084→1.0128, +0.0044; 2014 0.9609→0.9639, 2018 0.9903→0.9941, 2022 1.0741→1.0806). XGBoost already exploits the raw window means and has the `n_matches` feature to gauge their reliability itself — pre-shrinking flattens discriminative signal it was using. Reverted (flag default OFF, kept for the record).

**✅ R10 (DONE/KEPT 2026-06-15). Estimate μ and home-advantage from the data.** Replaced hard-coded `μ=log(1.3)` and `home_adv=0.20` with moment estimates from the goal data (att/def are zero-mean → no-home-term goals give E[goals]=exp(μ); non-neutral home goals give the extra exp(home_adv)); clamped to sane ranges and folded into the Kalman cache/fingerprint (`features/kalman_strength.py`, flag `USE_KALMAN_EM_MU_HA`, default ON). On this data μ̂≈0.174 (vs 0.262) and ĥ≈0.36 (vs 0.20). WC ablation: **improved all 3 folds** (avg 1.0084→1.0036, −0.0048; best single config). Within the ±0.1 CI but directionally consistent and principled (removes two hand-set hyperparameters) — same keep-standard as R5/R7. Not a deep EM M-step (the full Poisson-GLM-in-EM is heavier and the smoothed-state coupling adds risk for ~0 extra signal); this contained moment estimate captures the intent. Wide-set (266-match) re-confirmation advisable before treating the gain as established.
...
| 2026-06-15 | **R8/R9/R10 pass** (`scripts/roadmap_ablation.py`, WC set, fw=0.8; this run's baseline 1.0084) | WC 3-fold | **R10 μ/ha kept** (1.0084→1.0036, −0.0048, all 3 folds better); **R9 shrinkage rejected** (→1.0128, +0.0044, all 3 worse); R8 = submission optimiser, see its own EV note (not a log-loss change). |
| 2026-06-16 | **New signals** (`scripts/feature_ablation.py`; baseline R10-on) | WC 3-fold + wide 7-fold | **`qualification` kept** — qualifier-only ppg/GD/win-rate as-of-date; wide set 0.9473→**0.9464** (−0.0009, 5/7 folds better; WC18 +0.013 & AFCON +0.003 the only regressions), same standard as `rest`/`xG`. **`wc_pedigree` rejected** — as-of-date WC apps/win-rate worsened all 3 WC folds (1.0036→1.0168, +0.0132; overlaps ranking/confederation, team-ID proxy). Module kept in REGISTRY (off). Leakage-reviewed (qualification clean; pedigree's same-edition peek fixed to a prior-editions-only year cut). **Re-run `--tune --retune` to re-optimise XGB params + friendly_weight for the +qualification feature set.** |
